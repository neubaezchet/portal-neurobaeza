...
const CAT_META = {
  superan_180:       { icon: AlertTriangle, iconColor: 'text-red-600',    ringColor: 'ring-red-500/40',     barColor: '#ef4444' },
  cerca_180:         { icon: AlertCircle,   iconColor: 'text-orange-600', ringColor: 'ring-orange-500/40',  barColor: '#f97316' },
  con_gaps_criticos: { icon: Shield,        iconColor: 'text-amber-600', ringColor: 'ring-yellow-500/40',  barColor: '#eab308' },
  prorrogas_activas: { icon: Zap,           iconColor: 'text-blue-600',   ringColor: 'ring-blue-500/40',    barColor: '#3b82f6' },
        ...
        {data.map((d, i) => (
          <div key={i} className="group">
            <div className="flex items-center justify-between text-[10px] mb-0.5">
              <span className="text-slate-500 truncate max-w-[60%]">{d[labelKey]}</span>
              <span className="text-slate-900 font-bold">{d[valueKey]}</span>
            </div>
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-4">
      <h3 className="text-xs font-bold text-slate-900 mb-3 flex items-center gap-2">
        <BarChart3 className="w-4 h-4 text-violet-600" /> {title}
      </h3>
    ...
    <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-blue-600" /> Línea de Tiempo
        </h3>
          ...
          {timeline.map((t, i) => {
            ...
            return (
              <div key={i} className="absolute h-full rounded cursor-pointer transition-all hover:brightness-125 hover:ring-2 hover:ring-white/30 hover:z-20"
                ...
                {width > 3 && <span className="text-[8px] text-slate-900 font-bold px-1 truncate block leading-10">{t.dias}d</span>}
              </div>
            );
          })}
          {(gaps || []).map((g, i) => {
        ...
        {tooltip && (
          <div className="absolute top-20 bg-gray-950 border border-gray-600 rounded-lg p-3 shadow-2xl z-50 text-xs max-w-xs" style={{ left: `${Math.min(tooltip.x, 70)}%` }}>
            {tooltip.type === 'incap' ? (<>
              ...
              <p className="text-blue-600">{(tooltip.item.tipo || '').replace(/_/g, ' ')}</p>
              {tooltip.item.diagnostico && <p className="text-slate-700 mt-1">{tooltip.item.diagnostico.substring(0, 80)}</p>}
              {tooltip.item.codigo_cie10 && <p className="text-cyan-600 font-mono">{tooltip.item.codigo_cie10} — {tooltip.item.cie10_descripcion}</p>}
              {tooltip.cadena && <p className="text-violet-600 font-bold mt-1">&#128279; {tooltip.cadena}</p>}
            </>) : (<>
              <p className={`font-bold ${tooltip.item.corta_prorroga ? 'text-red-600' : 'text-amber-600'}`}>
                {tooltip.item.corta_prorroga ? '&#128308; GAP CRÍTICO — CORTA PRÓRROGA' : '&#128993; Hueco'}
              </p>
              <p className="text-slate-500">{tooltip.item.fecha_inicio} → {tooltip.item.fecha_fin}</p>
              <p className="text-slate-900 font-bold">{tooltip.item.dias} días sin cobertura</p>
              {tooltip.item.corta_prorroga && <p className="text-red-700 text-[10px] mt-1">Brecha &gt;30d: reinicia conteo</p>}
            </>)}
          </div>
        )}
      </div>
      <div className="px-4 py-2 border-t border-slate-200 flex justify-between text-[9px] text-slate-500 font-mono">
      ...
      <div className="overflow-x-auto max-h-[400px] overflow-y-auto">
        <table className="w-full text-xs">
          <thead className="bg-slate-50 sticky top-0 z-10">
            <tr>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Tipo</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Inicio</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Fin</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Días</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">CIE-10</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Serial</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Estado</th>
              <th className="px-3 py-2 text-left text-slate-500 font-semibold">Prórroga</th>
            </tr>
          </thead>
            ...
            {visible.map((item, i) => (
              item._type === 'gap' ? (
                <tr key={`g${i}`} className="bg-red-950/30 hover:bg-red-900/40">
                  <td className="px-3 py-2" colSpan={8}>
                    <div className="flex items-center gap-2">
                      ...
                      {item.corta_prorroga && <span className="text-red-600 text-[9px] ml-auto">Brecha &gt;30d</span>}
                    </div>
                  </td>
                </tr>
              ) : (
                <tr key={`i${i}`} className="hover:bg-gray-800/50">
                  ...
                  <td className="px-3 py-2 font-mono text-slate-700">{item.fecha_inicio}</td>
                  <td className="px-3 py-2 font-mono text-slate-700">{item.fecha_fin}</td>
                  <td className="px-3 py-2 font-bold text-slate-900">{item.dias}</td>
                  <td className="px-3 py-2 max-w-xs">
                    {item.codigo_cie10 && <span className="text-cyan-600 font-mono text-[10px] mr-1">[{item.codigo_cie10}]</span>}
                    <span className="text-slate-500 text-[10px]">{(item.cie10_descripcion || item.diagnostico || '').substring(0, 60)}</span>
                  </td>
                  <td className="px-3 py-2 font-mono text-blue-700 text-[10px]">{item.serial}</td>
...
function CadenasProrroga({ cadenas }) {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200">
        <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
          <Zap className="w-4 h-4 text-violet-600" /> Cadenas de Prórroga ({activas.length})
        </h3>
      </div>
        ...
        {activas.map((cad, i) => {
          ...
          return (
            <div key={i} className={`border rounded-xl p-4 ${sup ? 'border-red-500/60 bg-red-950/20' : cri ? 'border-orange-500/60 bg-orange-950/10' : ale ? 'border-yellow-500/50 bg-yellow-950/10' : 'border-gray-700/60 bg-gray-800/30'}`}>
              ...
              <div className="w-full bg-slate-200 rounded-full h-3 mb-2 overflow-hidden relative">
    ...
    <div className={`border rounded-xl overflow-hidden transition-all ${open ? `${meta.ringColor} ring-1` : 'border-gray-700/60'}`}>
      <button onClick={() => setOpen(!open)}
        ...
        <div className="flex-1">
          ...
          <p className="text-[10px] text-slate-500">{count} persona{count !== 1 ? 's' : ''}</p>
        </div>
      ...
      {open && (
        <div className="border-t border-slate-200 max-h-72 overflow-y-auto">
          <table className="w-full text-xs">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-slate-500">Incap.</th>
                <th className="px-3 py-2 text-left text-slate-500">Días</th>
                <th className="px-3 py-2 text-left text-slate-500">Max Cadena</th>
                <th className="px-3 py-2 text-left text-slate-500">Gaps</th>
                <th className="px-3 py-2 text-left text-slate-500">180d</th>
            ...
            <tbody className="divide-y divide-slate-200">
              {cat.personas.map((p, i) => (
                <tr key={i} className="hover:bg-gray-800/50 cursor-pointer group" onClick={() => onSelectPerson(p.cedula)}>
                  ...
                  <td className="px-3 py-2 text-blue-600 font-bold">{p.total_incapacidades}</td>
                  <td className="px-3 py-2 text-slate-900 font-bold">{p.total_dias}</td>
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-2">
                  ...
                  <td className="px-3 py-2">
                    {p.gaps_criticos > 0 ? (
                      <span className="text-red-600 font-bold">{p.gaps_criticos} &#128308;</span>
                    ) : p.total_gaps > 0 ? (
                      <span className="text-amber-600">{p.total_gaps}</span>
                    ) : <span className="text-slate-400">—</span>}
                  </td>
                  <td className="px-3 py-2">
                    <div className="bg-slate-200 rounded-full h-2.5 overflow-hidden" style={{ width: 60 }}>
        ...
        <>
          {globalLoading && (
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-12 text-center">
              ...
              <p className="text-slate-500 text-sm mt-3">Analizando todas las personas...</p>
              <p className="text-slate-400 text-xs mt-1">Detectando prórrogas, gaps y alertas de 180 días</p>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
                <h3 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">
                  <Activity className="w-4 h-4 text-amber-600" /> Clasificación por Estado de Prórroga y 180 Días
                </h3>
      ...
      {vista === 'individual' && (
        <>
          ...
          {searchResults && !personData && (
            searchLoading ? (
              ...
              <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-6 text-center">
                ...
                <p className="text-slate-500 text-sm mt-2">Sin resultados</p>
              </div>
            ) : (
              <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden">
                  ...
                  {searchResults.map((r, i) => (
                    <button key={i} onClick={() => loadPerson(r.cedula)}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-slate-900 truncate">{r.nombre}</p>
                        <p className="text-[10px] text-slate-500">CC {r.cedula} - {r.empresa} - {r.cargo || r.area || ''}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className="text-sm font-black text-blue-600">{r.total_incapacidades}</p>
                        <p className="text-[9px] text-slate-500">{r.total_dias}d</p>
                      </div>
          ...
          {personLoading && (
            <div className="bg-white border border-slate-200 shadow-sm rounded-xl p-12 text-center">
              <Loader2 className="w-10 h-10 animate-spin text-blue-600 mx-auto" />
              <p className="text-slate-500 text-sm mt-3">Analizando historial...</p>
            </div>
          )}
          ...
          {!personData && !searchResults && !searchLoading && !personLoading && (
            <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-12 text-center">
              ...
              <p className="text-slate-400 text-sm mt-2 max-w-md mx-auto">
                Busca por <strong className="text-slate-500">cédula</strong> o <strong className="text-slate-500">nombre</strong> para ver
